...
parser.add_argument('--nEpochs', type=int, default=2, help='number of epochs to train for') # 2048 default
...
parser.add_argument('--data_path', type=str, default='/data_scene_flow/training/', help="data root")
parser.add_argument('--training_list', type=str, default='./lists/kitti2015_train.list', help="training list")
parser.add_argument('--val_list', type=str, default='./lists/kitti2015_test.list', help="validation list")
parser.add_argument('--save_path', type=str, default='./checkpoint/kitti2015', help="location to save models")
...
test_set = get_test_set(opt.data_path, opt.val_list, [opt.crop_height, opt.crop_width], opt.left_right, opt.kitti, opt.kitti2015)
...
print("Number of parameters: {}".format(total_num))
...
parameters_to_prune = (
    (model.conv_start[0].conv, 'weight'),
    (model.conv_start[0].bn, 'weight'),
    (model.conv_start[1].conv, 'weight'),
    (model.conv_start[1].bn, 'weight'),
	(model.conv_x.conv, 'weight'),
	(model.conv_y.conv, 'weight'),
	(model.conv_x.bn, 'weight'),
	(model.conv_y.bn, 'weight'),
    (model.cost_agg.conv_start.conv, 'weight'),
	(model.cost_agg.conv_start.bn, 'weight'),
	(model.cost_agg.conv1a.conv, 'weight'),
	(model.cost_agg.conv1a.bn, 'weight'),
	(model.cost_agg.conv2a.conv, 'weight'),
	(model.cost_agg.conv2a.bn, 'weight'),
	(model.cost_agg.conv1b.conv1.conv, 'weight'),
	(model.cost_agg.conv1b.conv1.bn, 'weight'),
	(model.cost_agg.conv1b.conv2.conv, 'weight'),
	(model.cost_agg.conv1b.conv2.bn, 'weight'),
	(model.cost_agg.sga1.conv_refine.conv, 'weight'),
	(model.cost_agg.sga2.conv_refine.conv, 'weight'),
	(model.cost_agg.sga3.conv_refine.conv, 'weight'),
	(model.cost_agg.sga11.conv_refine.conv, 'weight'),
	(model.cost_agg.sga12.conv_refine.conv, 'weight'),
	(model.cost_agg.sga13.conv_refine.conv, 'weight'),
	(model.cost_agg.sga14.conv_refine.conv, 'weight'),
)
...
print(
    "Sparsity in conv1.weight: {:.2f}%".format(
        100. * float(torch.sum(model.conv_start[0].conv.weight == 0))
        / float(model.conv_start[0].conv.weight.nelement())
    )
)
...
if cuda == 0:
    ...
    # nr of MACs
    input = torch.randn(16, 3, 48, 144) # torch.randn([16, 3, 48, 144]) # x: [16, 3, 48, 144], y: [16, 3, 48, 144]
...
def train(epoch):
    ...
    for iteration, batch in enumerate(training_data_loader):
        ...
        if valid > 0:
            ...
            elif opt.model == 'GANet_deep' or opt.model == 'GANet_26':
...
if __name__ == '__main__':
    ...
    for epoch in range(1+epoch, opt.nEpochs + 1+epoch):
...
#        if opt.kitti or opt.kitti2015:
        ...
        if opt.kitti or opt.kitti2015:
            if (epoch%50 == 0 and epoch >= 25) or epoch == 1:
        ...
        print("Time consumed for the epoch is {}s".format(delta))
